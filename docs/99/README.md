...
나중에 토큰 기반 인증에서 다루겠지만 이와 같은 흐름이 필요할 수 있습니다. <b>그러나 여기서 말하는 토큰들은 foo가 발행한 토큰이 아니라 구글과 같은 OAuth2 서비스 제공자가 발행한 토큰들이고 외부 API 호출을 계속해야 한다는 점에서 큰 차이가 있습니다. 구글 외에 다른 소셜 로그인이 되면 더 복잡해질 것입니다.</b> 근본적으로 이 토큰들의 용도는 그런 것이 아님에도 계속 다른 목적으로 사용하려고 하기 때문에 안티 패턴들이 나타나는 것입니다. 
...
이렇게 되면 백엔드인 클라이언트에서 리프레시 토큰을 보관하는 것이 맞고 액세스 토큰 만료여부에 따라 새로운 액세스 토큰을 발급받는 주체는 클라이언트도 됩니다.  
...
새로운 토큰은 JWT의 포맷을 따르는 경우가 많습니다. 이때 JWT를 액세스 토큰과 리프레시 토큰 두개로 나누어 발행할 수 있습니다. 이 토큰들은 OAuth2 서비스 제공자의 그것과는 성격과 목적이 완전히 다릅니다. 맥락상 이 토큰들의 목적은 인증/인가 토큰이지만 차이가 있습니다. 액세스 토큰은 로그인 했음을 증명하는 인증 토큰이면서 동시에 해당 애플케이션의 리소스에 접근할 수 있는 인가 토큰의 역할도 할 수 있습니다. 리프레시 토큰은 만료시간을 늘려주기 위한 액세스 토큰 재발급용입니다. 이들 토큰은 내가 만드는 애플리케이션이 관리 합니다.
...
스프링 시큐리티의 `oauth2Login`의 간단한 구현은 [여기](https://github.com/boyd-dev/demo-security/blob/main/docs/06/README.md)를 참조하면 되겠습니다. 이 예제에서는 단순하게 인증만 하고 별도의 토큰을 발행하지 않습니다. 이 말은 인증 후에는 사용자 세션을 만든다는 말이 되겠습니다(로그아웃하면 액세스 토큰을 폐기하고 세션을 삭제합니다).
